...
@section('title', 'Сортировка массивов - Practice 8')
            ...
            <div class="card-body">
                <form id="sortForm">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea 
                            class="form-control @error('array') is-invalid @enderror" 
                            id="array" 
                            name="array" 
                            rows="3"
                            placeholder="Введите числа через запятую или пробел, например: 64, 34, 25, 12, 22, 11, 90"
                            required
                        >{{ old('array', '64, 34, 25, 12, 22, 11, 90') }}</textarea>
                        @error('array')
                    ...
                    <div class="mb-3">
                        ...
                        >
                            ...
                            @foreach($algorithms as $key => $name)
                                <option value="{{ $key }}" {{ old('algorithm') == $key ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="bg-light p-2 rounded">
                            <code>[{{ implode(', ', $result['original']) }}]</code>
                        </div>
                        
                        <h5 class="mt-3">Отсортированный массив:</h5>
                        <div class="bg-success bg-opacity-10 p-2 rounded">
                            <code>[{{ implode(', ', $result['sorted']) }}]</code>
                        </div>
                    </div>
...
document.getElementById('sortForm').addEventListener('submit', function(e) {
    ...
    fetch('{{ route("services.sort.sort") }}', {
        method: 'POST',
        body: formData,
        headers: {
...
function displayResult(result) {
    const algorithms = @json($algorithms);
