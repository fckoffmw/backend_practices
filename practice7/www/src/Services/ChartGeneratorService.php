...
/**
 * Сервис для генерации графиков с использованием GD
 */
class ChartGeneratorService
{
    private string $chartsPath;

    public function __construct(string $chartsPath)
    {
    ...
    public function generatePieChart(array $data, string $title): string
    {
        ...
        if (empty($data)) {
            ...
            // Общее количество
            $total = array_sum(array_column($data, 'sales_count'));
            ...
            foreach ($data as $item) {
                $angle = ($item['sales_count'] / $total) * 360;
                ...
                imagestring($image, 2, $labelX - 20, $labelY - 10, 
                    $item['region'] . ' (' . $item['sales_count'] . ')', $black);
